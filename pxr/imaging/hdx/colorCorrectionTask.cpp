...
HdxColorCorrectionTask::~HdxColorCorrectionTask()
{
    ...
    for (auto& buffer : _bufferConstants) {
        _hgi->DestroyBuffer(&buffer.handle);
    }
    _bufferConstants.clear();

    // Only for version 2 and above
    for (auto& textureLut : _textureLUTs) {
...
typedef
  std::vector<UniformBufferData> UniformBuffersData;

template<typename T, int nElements = 1>
void _SetConstantValue(UniformBuffersData& uniformData,
...
{
    UniformBuffersData uniformData;
...
std::string
HdxColorCorrectionTask::_CreateOpenColorIOShaderCode(std::string &ocioGpuShaderText, HgiShaderFunctionDesc &fragDesc)
{
    return "#define OCIO_DISPLAY_FUNC(inCol) OCIODisplay(inCol, Lut3DIn)";
}

void
HdxColorCorrectionTask::_CreateOpenColorIOLUTBindings(HgiResourceBindingsDesc &resourceDesc)
{
    if (_texture3dLUT) {
...
HdxColorCorrectionTask::_CreateOpenColorIOResources()
{
    ...
    for(int i = 0; i < shaderDesc->getNumTextures(); ++i) {
        ...
        int channelPerPix =
            channel == OCIO::GpuShaderCreator::TextureType::TEXTURE_RED_CHANNEL ? 1 : 4;
    ...
    auto uniformBuffersData = _GetUniformBuffersData(shaderDesc);
    _constantValues.reserve(1024);
    _constantValues.resize(sizeof(_screenSize));
    for(auto const& ubo : uniformBuffersData) {
        if(ubo.count == 1)
        {
...
HdxColorCorrectionTask::_CreateOpenColorIOShaderCode(std::string &ocioGpuShaderText, HgiShaderFunctionDesc &fragDesc)
{
    ...
    int bindingIdx = 1;
    for(auto const& texInfo : _textureLUTs) {
    ...
    for(auto const& buffInfo : _bufferConstants) {
        if(buffInfo.count == 1)
        {
            if(_hgi->GetAPIName() == HgiTokens->Metal)
            {
...
HdxColorCorrectionTask::_CreateOpenColorIOLUTBindings(HgiResourceBindingsDesc &resourceDesc)
{
    ...
    for (auto const &texSamp : _textureLUTs) {
        HgiTextureBindDesc texBind1;
    ...
    for(auto const& buff : _bufferConstants) {
        if(buff.count > 1)
        {
